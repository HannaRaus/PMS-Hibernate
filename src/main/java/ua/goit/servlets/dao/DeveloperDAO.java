...
public class DeveloperDAO extends AbstractDAO<Developer> {
    ...
    private void deleteProjects(List<Project> projectsInDB, List<Project> newProjects) throws DAOException {
        String query = "DELETE FROM project_developers WHERE project_id=?;";
    ...
    private void sendSkills(Developer developer) throws DAOException {
        ...
        deleteSkills(skillsInDB, newSkills);

        if (areNotEquals(skillsInDB, newSkills)) {
            try (Connection connection = getConnectionManager().getConnection();
                 PreparedStatement statement = connection.prepareStatement(query)) {
                newSkills.removeIf(skillsInDB::contains);
    ...
    private void deleteSkills(List<Skill> skillsInDB, List<Skill> newskills) throws DAOException {
        String query = "DELETE FROM developer_skills WHERE skill_id=?;";

        List<Skill> skillsToDelete = skillsInDB.stream()
                .filter(info -> !newskills.contains(info))
                .collect(Collectors.toList());
